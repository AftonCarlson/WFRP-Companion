...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        search_scope = scope.resolve_search_scope(
            ...
            source_set_id=args.source_set,
            book_ids=args.book_id,
            validate_book_ids=False,
        )
    except scope.SearchScopeConflictError as exc:
        parser.error(str(exc))
    except source_sets.SourceSetError as exc:
        print(str(exc), file=sys.stderr)
        return 1
    hits = search_exact(config, query, book_ids=search_scope.book_ids, limit=args.limit)
