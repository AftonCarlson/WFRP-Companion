...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Rebuild source-object search and FTS projections."
    )
    parser.add_argument(
        "--data-dir",
        type=Path,
    ...
    parser.add_argument(
        "--force",
        action="store_true",
        help="Rebuild even when the source-object search projection is current.",
    )
    parser.add_argument(
        "--retry-running",
        action="store_true",
        help="Recover all running source-object FTS rebuild jobs before rebuilding.",
    )
    parser.add_argument(
        "--stale-running-minutes",
        type=int,
        default=30,
        help="Recover running rebuild jobs older than this many minutes. Default: 30.",
    )
    return parser
...
def print_summary(config: AppConfig, summary: ObjectSearchRebuildSummary) -> None:
    print("WFRP source object FTS rebuild")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    summary = rebuild_source_object_search(
        config,
