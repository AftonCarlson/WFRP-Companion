...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Extract reviewable structured evidence candidates from WFRP books."
    )
    parser.add_argument(
        "--data-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "SQLite database path. Defaults to WFRP_DB_PATH or "
            "<data-dir>/wfrp_companion.sqlite."
        ),
    )
    parser.add_argument(
        "--book-id",
    ...
    parser.add_argument(
        "--force",
        action="store_true",
        help="Re-extract even when the current structured snapshot was extracted.",
    )
    parser.add_argument(
        "--retry-running",
        action="store_true",
        help="Recover all running structured-evidence extraction jobs first.",
    )
    parser.add_argument(
...
def print_summary(
    config: AppConfig,
    summary: StructuredEvidenceExtractionSummary,
) -> None:
    print("WFRP structured evidence extraction")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    summary = extract_structured_evidence_library(
        config,
